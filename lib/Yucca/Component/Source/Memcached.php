...
class Memcached extends SourceAbstract
{
     ...
     * @return \Yucca\Component\Source\Memcached
     */
    ...
    protected function getCacheKey($identifier){
        $toReturn = $this->prefix.'_'.$this->sourceName.'_'.(isset($this->configuration['version']) ? $this->configuration['version'] : 1);
    ...
    protected function getConnection(){
        ...
        if($toReturn instanceof \Memcached) {
            return $toReturn;
        }

        throw new \Exception("Connection binded to \"{$this->sourceName}\" must be a \"Memcached\" connection");
    }
    ...
    public function saveAfterLoading($datas, array $identifier=array(), $shardingKey=null, &$affectedRows=null) {
        $this->getConnection()->set($this->getCacheKey($identifier), $datas, 0);
    }
}
